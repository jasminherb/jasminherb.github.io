...
integrand(x) 함수가 정의되어 있습니다. 이 함수는 주어진 x에 대해 (g(x) - mean) ** 2 * f(x) 값을 반환합니다. 즉, g(x)와 mean의 차이의 제곱을 f(x)와 곱한 값을 반환합니다.

integrate.quad() 함수를 호출하여 integrand(x) 함수를 -np.inf부터 np.inf까지 수치적으로 적분합니다. integrate.quad() 함수는 첫 번째 요소로 적분의 추정 값, 두 번째 요소로 절대 오차의 추정 값을 반환합니다.
